...
//    browser).
//
// Replace FORMSPREE_ENDPOINT below if you ever rotate it.

const FORMSPREE_ENDPOINT = "https://formspree.io/f/xojrwzwo";

(function trackVisit() {
  try {
    // Don't ping more than once per browser session
    if (sessionStorage.getItem("anilove_visit_pinged")) return;
    sessionStorage.setItem("anilove_visit_pinged", "1");

    const data = new FormData();
    data.append("type", "visit");
    data.append("page", "anilove-waitlist-landing");
    data.append("ts", new Date().toISOString());
    data.append("ua", navigator.userAgent);
    data.append("ref", document.referrer || "(direct)");
    ...
    data.append("_subject", "👀 New visit on Anilove landing");
...
(function form() {
  ...
  form.addEventListener("submit", async (ev) => {
    ...
    fd.append("_subject", "💌 New Anilove waitlist signup");
